...
**Status:** B1 and B2 are complete, verified against live infrastructure, and frozen (`docs/adr/ADR-009-b1-platform-freeze.md`, `docs/adr/ADR-012-b2-platform-freeze.md`), tagged `b2-freeze`. B3 (Registry) is feature-complete per its Phase-0 scope — Slice 1 (Parcel Aggregate, ADR-013), Slice 2 (Atomic Parcel Number Allocation, ADR-014), Slice 3 (Mutation Commands & Authorization Hardening, ADR-015), and Slice 4 (Geometry Port Boundary & Spatial Integration Foundation, `docs/adr/ADR-016-geometry-port-boundary-spatial-integration.md`) are all implemented, and the **B3 Final Quality Gate has passed** — full `ruff`/`mypy`/`pytest` plus live Postgres/Keycloak/RLS/delegation/audit-chain/cross-tenant/ownership-attack/container verification, see `docs/B3_FINAL_VERIFICATION_CHECKLIST.md` for full evidence. B3 is ready to be proposed for Platform Freeze pending explicit review. B4 (Spatial Intelligence) is not authorized. See `CLAUDE.md`'s "B2 status"/"B3 status" and `docs/audits/B2_RELEASE_NOTES.md` for exactly what's landed.
...
| [`docs/adr/`](./docs/adr/) | Architecture Decision Records — ADR-001 through ADR-016 |
