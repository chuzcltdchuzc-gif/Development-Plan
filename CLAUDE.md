...
Docker Compose (Postgres + Keycloak + backend + frontend) has been booted end-to-end and is the normal way this repo is verified now — see `docs/audits/B1_INFRASTRUCTURE_VERIFICATION.md` for the full live-infrastructure validation this passed (migrations, RLS, JWT, rate limiting, audit chain, adversarial security checks). Cloud (staging/production) environments do not exist yet. **Compute provider: AWS** (decided 2026-07-30; `infra/terraform/versions.tf` now declares the `hashicorp/aws` provider, region only, no resources yet — not run through `terraform validate`, since Terraform isn't installed in the environment that authored it). Formally captured at `docs/adr/ADR-024-delivery-platform-and-infrastructure-decisions.md` (Proposed, 2026-07-30) — Delivery Platform & Infrastructure Decisions, covering StoragePort/R2/WORM, Keycloak, Paystack, this AWS decision, and the still-undecided secrets manager. This note is the pointer; the ADR is the source of truth.
