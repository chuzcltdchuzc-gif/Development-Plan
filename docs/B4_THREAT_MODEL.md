...
architectural requirements** that ADR-018 through ADR-023 must satisfy — it does not itself
decide the schema, the query design, or the exact authorization mechanism (those remain ADR-018
through ADR-022's job). Where this document says "must," it is stating a constraint the
...
- **TB5 — The cross-tenant read boundary inside Spatial Intelligence itself (the crux of this
  ...
  boundary does not yet have a design (that is ADR-021's job) — this document's job is to state
...
- **Tampering:** the adapter must not be able to mutate `parcel.geometry_reference` or any other
  ...
  boolean answer (ADR-016's existing design). **Requirement for ADR-020:** the real adapter's
...
- **Information disclosure:** an adapter that raises an exception with detail about *why* a
  ...
  ADR-020:** `reference_is_valid`'s failure mode must be a plain boolean; any richer diagnostic
...
- **Denial of service:** a slow or unbounded adapter call blocks the request thread inside
  ...
  **Requirement for ADR-020:** the real adapter's validation must be boundable in cost (e.g., no
...
- Standard PDP/PEP threats, structurally identical to every prior context's own TB2-equivalent —
  no novel threat identified here **provided** ADR-022 in fact reuses the existing pipeline
  ...
  **elevation of privilege via a "helpful" spatial-specific bypass** — e.g., a future engineer
  ...
  informal exception, without an ADR. **Requirement for ADR-022:** any such exception must be
...
- **Information disclosure (the primary threat):** the most direct way this feature could go
  ...
  **Requirement for ADR-021:** the response to an ordinary registrant must be minimal — a
  ...
  decision ADR-021/022 must make explicitly, not an assumed consequence of "governance roles see
...
- **Elevation of privilege (the mechanism-level threat):** whatever code path performs the
  ...
  rolled back. **Requirement for ADR-021:** any cross-tenant read the overlap computation
...
- **Tampering:** a malicious or malformed geometry payload (self-intersecting polygon, absurd
  ...
  ADR-020:** payload validation (well-formedness, coordinate bounds, no self-intersection) must
...
- **Denial of service:** an unindexed or naively-designed overlap query is an O(n) or worse scan
  ...
  submission. **Requirement for ADR-021:** GiST indexing and query design must be evaluated
...
- **Spoofing:** GPS data itself can be spoofed (`identity_users`-adjacent precedent: this
  ...
  *solve* GPS spoofing (that's a data-quality/Trust-Engine-scoring concern, arguably B9/B7's
  territory), but ADR-020's validation rules must not silently treat a spoofed or degenerate
...
- **Repudiation:** every geometry submission and every overlap determination must be audited
  ...
  ADR-018/021:** no second audit mechanism, and the overlap-check audit entry itself must not leak
...
- No implementation exists to analyze yet. The one requirement worth stating now, before either
  ...
  interface ADR-021 defines.

## 6. Summary of binding requirements carried forward into ADR-018 through ADR-022
...
2. **ADR-020 (Real Adapter & Validation):** `reference_is_valid`'s failure mode is a plain
...
3. **ADR-021 (Overlap Detection):** the single most consequential ADR from a security standpoint.
...
4. **ADR-022 (Spatial Authorization):** reuses the existing pipeline verbatim (TB4); any
...
- The exact shape of "minimal disclosure on conflict" (§5/TB5) is a product/legal question as
  ...
  registrant to see is domain input this document cannot supply; ADR-021 must either resolve it
...
- Whether the overlap-detection computation should run synchronously (blocking the registration
  ...
  ADR-021 decision, not decided here.
...
Spatial Domain Model begins only after this document is reviewed and approved.** ADR-020 through
ADR-022 must each satisfy the binding requirements in §6 to be accepted; ADR-021 in particular
